...
public class LOF extends Filter implements OptionHandler,
  ...
  protected class NNFinder implements Runnable {
    ...
    protected boolean m_trimZeroDistances;
    protected Logger LOGGER = Logger.getSingleton();
    ...
    public NNFinder(Instances training, int start, int end,
    	      NearestNeighbourSearch search, boolean trimZeroDistances) {
    	      m_nnTrain = training;
    	      m_start = start;
    	      m_end = end;
    	      m_search = search;
    	      m_trimZeroDistances = trimZeroDistances;
    	    }
        ...
        for (int i = m_start; i < m_end; i++) {
          ...
          if (addToKDistanceContainer(key, n)) {
            ...
            do {
             ...
             // Logger.log(Logger.Level.INFO, "Num of Neighbours before TrimZeroDistances: "+nn.numInstances());
              n.m_neighbors = nn;
              n.m_distances = m_search.getDistances();
              if(m_trimZeroDistances){
            	  trimZeroDistances(n);
              }
  ...
  protected void LOFFirstBatchParallel() throws Exception {
    ...
    for (int i = 0; i < m_numExecutionSlots; i++) {
      ...
      NNFinder finder = new NNFinder(new Instances(training), start, end, s, true);
